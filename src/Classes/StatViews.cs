    ...
    public sealed class StatViews : IFlickrParsable
    {
        ...
        void IFlickrParsable.Load(XmlReader reader)
        {
            ...
            while (reader.Read() && reader.LocalName != "stats")
            {
                ...
                {
                    case "total":
                        TotalViews = int.Parse(reader.GetAttribute("views"), NumberFormatInfo.InvariantInfo);
                        break;
                    case "photos":
                        PhotoViews = int.Parse(reader.GetAttribute("views"), NumberFormatInfo.InvariantInfo);
                        break;
                    case "photostream":
                        PhotostreamViews = int.Parse(reader.GetAttribute("views"), NumberFormatInfo.InvariantInfo);
                        break;
                    case "sets":
                        PhotosetViews = int.Parse(reader.GetAttribute("views"), NumberFormatInfo.InvariantInfo);
                        break;
                    case "collections":
                        CollectionViews = int.Parse(reader.GetAttribute("views"), NumberFormatInfo.InvariantInfo);
                        break;
                    case "galleries":
                        GalleryViews = int.Parse(reader.GetAttribute("views"), NumberFormatInfo.InvariantInfo);
                        break;
                    default:
                        UtilityMethods.CheckParsingException(reader);
                        break;
